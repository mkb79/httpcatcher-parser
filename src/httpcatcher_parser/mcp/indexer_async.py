...
class AsyncKeyIndexer:
    """Manages normalized key lookup with caching (async version)."""

    def __init__(self, db: AsyncDatabase, prefetch: bool = True):
        self.db = db
...
class AsyncIndexer:
    ...
    def __init__(self, db: AsyncDatabase, scanner: HttpCatcherScanner):
        self.db = db
    ...
    async def index_file(self, file_path: Path, force_reindex: bool = False) -> dict:
        ...
        # Import AsyncFileTracker here to avoid circular import
        from .file_tracker_async import AsyncFileTracker
        file_tracker = AsyncFileTracker(self.db)
    ...
    async def _bulk_insert(self, file_id: int, aggregations: dict[int, RequestAggregation]) -> int:
        ...
        key_indexer = AsyncKeyIndexer(self.db)
